 ...
 */
  ...
  function renderBudgets() {
    ...
    tbody.innerHTML = list.map((budget) => {
      ...
      return `
        <tr>
          <td data-label="Número"><button type="button" class="budget-number budget-record-open" data-action="edit" data-id="${budget.id}" title="Abrir orçamento ${escapeHtml(budget.codigo)}"><i class="fa-regular fa-file-lines"></i><span>${escapeHtml(budget.codigo)}</span></button></td>
          <td data-label="Emissão">${escapeHtml(localDate(budget.data_emissao))}</td>
          <td data-label="Cliente"><div class="budget-client-cell"><button type="button" class="budget-client-open budget-record-open" data-action="edit" data-id="${budget.id}" title="Abrir orçamento de ${escapeHtml(budget.cliente_nome || 'Cliente não vinculado')}"><strong>${escapeHtml(budget.cliente_nome || 'Cliente não vinculado')}</strong></button><small>${escapeHtml(budget.cliente_documento || '')}</small></div></td>
