...
import { obterCampoFornecedor, salvarCampoFornecedor } from './api.js';
...
export async function openFieldModalEdit(id) {
  try {
    const campo = await obterCampoFornecedor(id);
...
export async function saveCampo() {
  ...
  try {
    ...
    await salvarCampoFornecedor(payload, state.campoEditandoId);
    await _afterSave();
    closeFieldModal();
