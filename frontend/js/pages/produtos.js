...
(() => {
  ...
  function isProdutoMoneyValue(value) {
    const text = String(value ?? '').trim();
    if (!text) return true;
    if (text.length > 40) return false;
    return /^(?:R\$\s*)?[+-]?\d[\d\s.,]*$/i.test(text.replace(/\u00a0/g, ' ').trim());
  }
  ...
  function buildProdutoBaseFromCustom(customFields, fallback = {}) {
    ...
    const precoVendaRaw = getCustomValue(custom, [
      'preco_venda',
      'preco_de_venda',
    ...
    const precoVenda = isProdutoMoneyValue(precoVendaRaw)
      ? precoVendaRaw
      : String(fallback.preco_venda || '').trim();
